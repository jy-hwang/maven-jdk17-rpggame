...
public class PlaceholderEffect implements GameEffect {
    ...
    public PlaceholderEffect(GameEffectType type, int value, String customMessage) {
        ...
        logger.debug("PlaceholderEffect 생성: {} (값: {}, 메시지: {})", 
                    this.type, this.value, customMessage != null ? "커스텀" : "기본");
    }
    ...
    public boolean isPercentage() {
        // 퍼센트 타입인지 확인
        return type == GameEffectType.HEAL_HP_PERCENT || 
               type == GameEffectType.HEAL_MP_PERCENT;
    }
    ...
    private String generateApplyMessage(String targetName) {
        ...
        return switch (type) {
            case BUFF_ATTACK, BUFF_DEFENSE, BUFF_SPEED ->
                String.format("🚧 %s %s %s 효과 (미구현)", emoji, targetName, typeName);
                
            case CURE_POISON, CURE_PARALYSIS, CURE_SLEEP, CURE_ALL ->
                String.format("🚧 %s %s %s 효과 (미구현)", emoji, targetName, typeName);
                
            case TELEPORT ->
                String.format("🚧 %s %s 순간이동 효과 (미구현)", emoji, targetName);
                
            case REVIVE ->
                String.format("🚧 %s %s 부활 효과 (미구현)", emoji, targetName);
                
            case FULL_RESTORE ->
                String.format("🚧 %s %s 완전 회복 효과 (미구현)", emoji, targetName);
    ...
    public String getDebugInfo() {
        return String.format("PlaceholderEffect{type=%s, value=%d, customMessage='%s'}", 
                           type, value, customMessage != null ? customMessage : "없음");
    }
