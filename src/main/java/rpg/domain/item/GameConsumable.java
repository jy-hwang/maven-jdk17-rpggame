...
public class GameConsumable extends GameItem {
  ...
  public GameConsumable(String id, String name, String description, int value, ItemRarity rarity, int hpRestore, int expGain, boolean stackable) {
    ...
    if (hpRestore > GameConstants.NUMBER_ZERO) {
      this.effects.add(new SimpleHealEffect("HP", hpRestore));
    }
    if (expGain > GameConstants.NUMBER_ZERO) {
  ...
  public GameConsumable(String id, String name, String description, int value, ItemRarity rarity, int hpRestore, int mpRestore, int expGain, boolean stackable) {
    ...
    if (mpRestore > GameConstants.NUMBER_ZERO) {
  ...
  // 쿨다운 관련 메서드들 (나중에 구현)
  private boolean isOnCooldown(Player character) {
    // TODO: 캐릭터의 아이템 쿨다운 상태 확인
    return false;
  }

  private void applyCooldown(Player character) {
