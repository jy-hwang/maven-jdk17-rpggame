...
public class QuestTemplateLoader {
  ...
  private static Map<String, QuestTemplateData> loadQuestTemplates(String configPath, String category) {
      try {
          ...
          // Map으로 변환
          Map<String, QuestTemplateData> questMap = questList.stream()
              .collect(Collectors.toMap(QuestTemplateData::getId, template -> template));
   ...
   * 특정 레벨 범위의 퀘스트 필터링
   */
  public static Map<String, QuestTemplateData> filterByLevelRange(Map<String, QuestTemplateData> templates, 
  ...
  public static void printLoaderStatus() {
      ...
      // 설정 파일 존재 여부 확인
      System.out.println("\n🔍 설정 파일 상태:");
      System.out.printf("  main-quests.json: %s%n", 
          isConfigFileExists(MAIN_QUESTS_CONFIG) ? "✅ 존재" : "❌ 없음");
      System.out.printf("  side-quests.json: %s%n", 
          isConfigFileExists(SIDE_QUESTS_CONFIG) ? "✅ 존재" : "❌ 없음");
      System.out.printf("  daily-quests.json: %s%n", 
          isConfigFileExists(DAILY_QUESTS_CONFIG) ? "✅ 존재" : "❌ 없음");
      System.out.printf("  weekly-quests.json: %s%n", 
          isConfigFileExists(WEEKLY_QUESTS_CONFIG) ? "✅ 존재" : "❌ 없음");
