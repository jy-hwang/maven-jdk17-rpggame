...
public class JsonBasedQuestFactory {
    ...
    private Map<String, QuestTemplateData> filterByCategory(String category) {
        return allTemplates.entrySet().stream()
    ...
    public Quest createLevelAppropriateQuest(int playerLevel) {
        // 플레이어 레벨에 맞는 템플릿 찾기
        List<QuestTemplateData> suitableTemplates = allTemplates.values().stream()
            .filter(template -> template.getRequiredLevel() <= playerLevel)
        ...
        // 랜덤 선택
        QuestTemplateData selectedTemplate = suitableTemplates.get(
            (int) (Math.random() * suitableTemplates.size())
        );
    ...
    public Quest createRandomQuest(Quest.QuestType type, int playerLevel) {
        List<QuestTemplateData> typeTemplates = allTemplates.values().stream()
            .filter(template -> type.name().equals(template.getType()))
            .filter(template -> template.getRequiredLevel() <= playerLevel)
            .collect(Collectors.toList());
        ...
        QuestTemplateData selectedTemplate = typeTemplates.get(
            (int) (Math.random() * typeTemplates.size())
        );
    ...
    private QuestTemplateData findDailyTemplate(Quest.QuestType type) {
        return dailyQuestTemplates.values().stream()
            .filter(template -> type.name().equals(template.getType()))
            .findFirst()
            .orElse(null);
    }
    ...
    private QuestTemplateData findWeeklyTemplate(Quest.QuestType type) {
        return weeklyQuestTemplates.values().stream()
            .filter(template -> type.name().equals(template.getType()))
            .findFirst()
            .orElse(null);
    }
    ...
    private Quest createDynamicLevelQuest(int playerLevel) {
        ...
        Quest dynamicQuest = new Quest(
            questId, title, description, Quest.QuestType.KILL, 
            playerLevel, objectives, reward
        );
    ...
    public List<String> getAllMainQuestIds() {
        return mainQuestTemplates.keySet().stream()
            .sorted()
            .collect(Collectors.toList());
    }
    ...
    public List<String> getAllSideQuestIds() {
        return sideQuestTemplates.keySet().stream()
            .sorted()
            .collect(Collectors.toList());
    }
    ...
    public List<String> getAvailableQuestIds(int playerLevel) {
        return allTemplates.entrySet().stream()
            .filter(entry -> entry.getValue().getRequiredLevel() <= playerLevel)
    ...
    public int getQuestCountByType(Quest.QuestType type) {
        return (int) allTemplates.values().stream()
            .filter(template -> type.name().equals(template.getType()))
            .count();
    }
    ...
    public boolean validateTemplates() {
        ...
        for (Map.Entry<String, QuestTemplateData> entry : allTemplates.entrySet()) {
            ...
            try {
                // 기본 검증
                if (template.getId() == null || template.getTitle() == null || 
                    template.getType() == null || template.getObjectives() == null) {
    ...
    public void printFactoryStatus() {
        ...
        // 레벨별 분포
        System.out.println("\n📈 레벨별 분포:");
        Map<Integer, Long> levelDistribution = allTemplates.values().stream()
            .collect(Collectors.groupingBy(QuestTemplateData::getRequiredLevel, Collectors.counting()));
        ...
        // 연동 상태
        System.out.println("\n🔗 연동 상태:");
        System.out.printf("   GameItemFactory: %s%n", 
            GameItemFactory.getInstance() != null ? "✅ 연결됨" : "❌ 연결안됨");
        System.out.printf("   QuestTemplateConverter: %s%n", 
            converter != null ? "✅ 연결됨" : "❌ 연결안됨");
    ...
    public void printQuestTemplateDetails(String questId) {
        ...
        System.out.println("\n목표:");
        template.getObjectives().forEach((key, value) -> 
        ...
        if (reward.getItems() != null && !reward.getItems().isEmpty()) {
            ...
            reward.getItems().forEach(item -> 
                System.out.printf("     %s x%d (%s)%n", item.getItemId(), item.getQuantity(), item.getRarity()));
        }
    ...
    public void printAllQuests() {
        ...
        System.out.println("\n📖 메인 퀘스트:");
        mainQuestTemplates.forEach((id, template) -> 
            System.out.printf("   %s - %s (레벨 %d)%n", id, template.getTitle(), template.getRequiredLevel()));
        
        System.out.println("\n📋 사이드 퀘스트:");
        sideQuestTemplates.forEach((id, template) -> 
            System.out.printf("   %s - %s (레벨 %d)%n", id, template.getTitle(), template.getRequiredLevel()));
        
        System.out.println("\n⏰ 일일 퀘스트:");
        dailyQuestTemplates.forEach((id, template) -> 
            System.out.printf("   %s - %s (%s)%n", id, template.getTitle(), template.getType()));
        
        System.out.println("\n📅 주간 퀘스트:");
        weeklyQuestTemplates.forEach((id, template) -> 
            System.out.printf("   %s - %s (%s)%n", id, template.getTitle(), template.getType()));
