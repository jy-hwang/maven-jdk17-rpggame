...
import rpg.domain.item.effect.GainExpEffect;
import rpg.domain.item.effect.GameEffect;
import rpg.domain.item.effect.GameEffectType;
import rpg.domain.item.effect.HealHpEffect;
import rpg.domain.item.effect.HealMpEffect;
import rpg.domain.item.effect.PlaceholderEffect;
...
public class GameEffectFactory {
  ...
  private static GameEffect createSingleEffect(GameEffectData effectData) {
    ...
    if (value < GameConstants.NUMBER_ZERO) {
  ...
  public static GameEffect createRandomEffect() {
    ...
    if (implementedTypes.isEmpty()) {
      return createHealHpEffect(GameConstants.NUMBER_FIFTY); // 기본값
    }

    Random random = new Random();
    GameEffectType randomType = implementedTypes.get(random.nextInt(implementedTypes.size()));
    int randomValue = GameConstants.NUMBER_TWENTY + random.nextInt(GameConstants.NUMBER_EIGHTY_ONE); // 20-100
  ...
  public static GameEffect createRandomEffectForLevel(int level) {
    ...
    int baseValue = switch (randomType) {
      case HEAL_HP, HEAL_MP -> GameConstants.NUMBER_THIRTY + (level * GameConstants.NUMBER_FIVE);
      case HEAL_HP_PERCENT, HEAL_MP_PERCENT -> Math.min(GameConstants.NUMBER_FIFTY, GameConstants.NUMBER_TEN + (level * GameConstants.NUMBER_TWO));
      case GAIN_EXP -> GameConstants.NUMBER_FIFTY + (level * GameConstants.NUMBER_TEN);
      default -> GameConstants.NUMBER_FIFTY;
    };
  ...
  public static boolean validateEffect(GameEffectData effectData) {
    ...
    return switch (effectType) {
      case HEAL_HP_PERCENT, HEAL_MP_PERCENT -> value > GameConstants.NUMBER_ZERO && value <= GameConstants.NUMBER_HUNDRED;
      default -> value >= GameConstants.NUMBER_ZERO;
    };
  }
  ...
  public static List<String> validateEffects(List<GameEffectData> effectDataList) {
    ...
    for (int i = GameConstants.NUMBER_ZERO; i < effectDataList.size(); i++) {
