...
public class DynamicQuestDataProvider {
  ...
  public MonsterData selectRandomMonsterForLevel(int playerLevel) {
      ...
      if (suitableMonsters.isEmpty()) {
          // 레벨 범위를 넓혀서 재시도
          suitableMonsters = MonsterDataLoader.getMonstersByLevelRange(
              Math.max(1, playerLevel - 3), 
              playerLevel + 3
          );
      }
  ...
  public MonsterData selectMonsterByRarity(String rarity, int playerLevel) {
      List<MonsterData> monsters = MonsterDataLoader.getMonstersByRarity(rarity)
          .stream()
          .filter(monster -> Math.abs(monster.getMinLevel() - playerLevel) <= 5)
  ...
  public List<MonsterData> selectMultipleMonsters(int count, int playerLevel) {
      ...
      return allSuitable.stream()
          .sorted((a, b) -> random.nextBoolean() ? 1 : -1) // 랜덤 셔플
          .limit(count)
          .collect(Collectors.toList());
  }
  ...
  public GameItemData selectItemByType(String itemType) {
      List<GameItemData> typeItems = ConfigDataLoader.loadAllItems().values()
          .stream()
          .filter(item -> itemType.equalsIgnoreCase(item.getType()))
  ...
  public GameItemData selectItemByRarity(String rarity) {
      List<GameItemData> rarityItems = ConfigDataLoader.loadAllItems().values()
          .stream()
          .filter(item -> rarity.equalsIgnoreCase(item.getRarity().name()))
          .filter(this::isCollectable)
          .collect(Collectors.toList());
  ...
  public LocationData selectLocationByDifficulty(String difficulty, int playerLevel) {
      List<LocationData> difficultyLocations = LocationManager.getAllLocations()
          .stream()
          .filter(loc -> difficulty.equalsIgnoreCase(loc.getDangerLevel().name()))
          .filter(loc -> playerLevel >= loc.getMinLevel())
          .collect(Collectors.toList());
  ...
  private List<GameItemData> getCollectableItems() {
      return ConfigDataLoader.loadAllItems().values()
          .stream()
          .filter(this::isCollectable)
          .collect(Collectors.toList());
  }
