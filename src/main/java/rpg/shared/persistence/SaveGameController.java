...
public class SaveGameController {
   ...
   * 메인 저장/로드 메뉴 실행
   */
  public SaveLoadResult showSaveLoadMenu(Player player, GameState gameState, long gameStartTime) {
    while (true) {
      System.out.println("\n=== 💾 저장/로드 관리 ===");
      ...
      switch (choice) {
        case 1:
          saveGame(player, gameState, gameStartTime);
          break;
        case 2:
...
//        }
  ...
  public SaveLoadResult loadFromSlot() {
    try {
      ...
      boolean confirmLoad = InputValidator.getConfirmation("현재 게임을 이 캐릭터로 교체하시겠습니까? (현재 진행사항은 저장되지 않습니다)");
      ...
      if (confirmLoad) {
        ...
        return new SaveLoadResult(SaveLoadResult.ResultType.SUCCESS, saveData.getCharacter(), saveData.getGameState(), slotNumber);
      } else {
        return new SaveLoadResult(SaveLoadResult.ResultType.CANCELLED, null, null, 0);
      }
  ...
  public void showSaveFileInfo() {
    ...
    if (occupiedSlots > 0) {
      ...
      // 총 플레이 시간
      int totalPlayTime = slots.stream().filter(SaveSlotInfo::isOccupied).mapToInt(SaveSlotInfo::getPlayTime).sum();
