...
public class ItemDataLoader {
 ...
 public static void loadAllItemData() {
     ...
     try {
         ...
         logger.info("아이템 데이터 로드 완료: 보물 {}개, 장비 {}개, 드롭 {}개 (총 {}개)", 
                    treasureItems.size(), equipmentItems.size(), dropItems.size(), totalItems);
 ...
 private static Map<String, GameItemData> loadItemsFromFile(String filePath) {
     try (InputStream inputStream = ItemDataLoader.class.getResourceAsStream(filePath)) {
         ...
         return itemList.stream()
             .collect(Collectors.toMap(GameItemData::getId, item -> item));
 ...
 public static GameItem generateRandomTreasureItem() {
     ...
     // 보물 카테고리 아이템들만 필터링
     List<GameItemData> treasureList = treasureItems.values().stream()
         .filter(item -> "treasure".equals(getItemCategory(item)))
 ...
 public static GameItem generateSpecialMerchantItem() {
     ...
     merchantItems.addAll(treasureItems.values().stream()
         .filter(item -> "merchant".equals(getItemCategory(item)))
         .collect(Collectors.toList()));
     merchantItems.addAll(equipmentItems.values().stream()
         .filter(item -> "merchant".equals(getItemCategory(item)))
         .collect(Collectors.toList()));
 ...
 public static GameItem generateRandomDropItem() {
     ...
     List<GameItemData> commonDrops = dropItems.values().stream()
         .filter(item -> "COMMON".equals(item.getRarity().getDisplayName()) || 
                       "UNCOMMON".equals(item.getRarity().getDisplayName()))
         .collect(Collectors.toList());
 ...
 public static void printItemStatistics() {
     ...
     // 등급별 통계
     Map<String, Long> rarityStats = getAllItems().stream()
         .collect(Collectors.groupingBy(item -> item.getRarity().getDisplayName(), Collectors.counting()));
 ...
 private static GameItemData selectItemByRarity(List<GameItemData> items) {
     ...
     // 희귀도별 가중치 계산
     Map<String, Double> rarityWeights = Map.of(
         "COMMON", 0.5,
         "UNCOMMON", 0.3,
         "RARE", 0.15,
         "EPIC", 0.04,
     ...
     double totalWeight = items.stream()
         .mapToDouble(item -> rarityWeights.getOrDefault(item.getRarity().getDisplayName(), 0.1))
         .sum();
 ...
 private static GameConsumable createConsumableItem(GameItemData itemData) {
     try {
         ...
         GameConsumable fallback = new GameConsumable(
             itemData.getName(),
             itemData.getDescription(),
             itemData.getValue(),
             itemData.getRarity(),
 ...
 private static GameEquipment createEquipmentItem(GameItemData itemData) {
     ...
     try {
         equipType = GameEquipment.EquipmentType.valueOf(
             equipTypeStr != null ? equipTypeStr : "WEAPON"
         );
 ...
 private static int extractEffectValue(GameItemData itemData, String effectType) {
     ...
     return itemData.getEffects().stream()
         .filter(effect -> effect != null && effectType.equals(effect.getType()))
         .mapToInt(effect -> effect.getValue())
         .findFirst()
         .orElse(0);
 }
