...
import model.monster.MonsterRewards;
...
public class MonsterDataLoader {
  ...
  public static Map<String, MonsterData> loadAllMonsters() {
      ...
      try {
          // 기본 몬스터 데이터 로드
          Map<String, MonsterData> monsters = loadMonstersFromFile(BaseConstant.MONSTERS_CONFIG);
          
          // 지역별 몬스터 데이터 로드
          monsters.putAll(loadMonstersFromFile(BaseConstant.FOREST_MONSTERS_CONFIG));
          monsters.putAll(loadMonstersFromFile(BaseConstant.CAVE_MONSTERS_CONFIG));
          monsters.putAll(loadMonstersFromFile(BaseConstant.MOUNTAIN_MONSTERS_CONFIG));
          monsters.putAll(loadMonstersFromFile(BaseConstant.SPECIAL_MONSTERS_CONFIG));
  ...
  private static Map<String, MonsterData> loadMonstersFromFile(String filePath) {
      try (InputStream inputStream = MonsterDataLoader.class.getResourceAsStream(filePath)) {
          ...
          return monsterList.stream()
              .collect(Collectors.toMap(MonsterData::getId, monster -> monster));
  ...
  public static List<MonsterData> getMonstersByLocationAndLevel(String location, int playerLevel) {
      return getMonstersByLocation(location).stream()
          .filter(monster -> playerLevel >= monster.getMinLevel() && playerLevel <= monster.getMaxLevel())
          .collect(Collectors.toList());
  }
  ...
  private static Map<String, MonsterData> createDefaultMonsters() {
      ...
      // 기본 몬스터들
      defaultMonsters.put("SLIME", new MonsterData(
          "SLIME", "슬라임", "젤리 같은 몬스터",
          new MonsterStats(20, 5, 2, 3, 0.1),
          new MonsterRewards(10, 5, List.of()),
          List.of("숲속 깊은 곳"), 1, 3, 0.8, "COMMON", List.of(), Map.of()
      ));

      defaultMonsters.put("GOBLIN", new MonsterData(
          "GOBLIN", "고블린", "작고 교활한 몬스터",
          new MonsterStats(30, 8, 3, 5, 0.15),
          new MonsterRewards(15, 10, List.of()),
          List.of("숲속 깊은 곳", "어두운 동굴"), 2, 5, 0.7, "COMMON", List.of(), Map.of()
      ));
  ...
  public static void printMonsterStatistics() {
      ...
      // 등급별 통계
      Map<String, Long> rarityStats = monsterCache.values().stream()
          .collect(Collectors.groupingBy(MonsterData::getRarity, Collectors.counting()));
      
      System.out.println("\n📊 등급별 분포:");
      rarityStats.forEach((rarity, count) -> 
          System.out.printf("   %s: %d개%n", rarity, count));

      // 지역별 통계
      System.out.println("\n🗺️ 지역별 분포:");
      locationCache.forEach((location, monsters) -> 
          System.out.printf("   %s: %d종류%n", location, monsters.size()));
